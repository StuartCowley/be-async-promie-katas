...
const food = () => {
  return fetch("food").then((food) => { return food.data })
};
...
const cat = () => {
  return fetch("cats").then((cat) => { return cat.data.cats.sort() });
};
...
const dog = () => {
  return fetch("dogs").then((dog) => {
    const naughtyDog = dog.data.dogs.reduce((a, b) => a.naughty > b.naughty ? a : b);
    return naughtyDog;
   });
};
...
const joke = () => {
  ...
  const response = Promise.all([question, answer])
  .then((values) =>  { 
    const question = values[0];
    const answer = values[1];
    const questionKey = "joke";
    const answerKey = "answer";
    
    return { question: question[questionKey], answer: answer[answerKey] }
  });
  return response;
};
